...
| 4 | 巡线到 B 并保持钢球中心 |
| 5 | 巡线一圈并保持钢球中心 |
| 6 | 先用 `6set` 锁定当前位置，再用 `6run` 巡线并保持该位置 |
...
|---:|---|---|
| 0 | 系统 | 绿=正常，红快闪=故障 |
| 1 | UART/MSPM0 | 黄呼吸=等待通信，绿=已收数据，红快闪=解码错误 |
| 2 | Y42 回零 | 黄呼吸=回零中，红快闪=失败，灭=未回零 |
| 3 | 相机/视觉 | 黄呼吸=等待视觉帧，绿=视觉帧正常，红快闪=相机或视觉错误 |
| 4 | 钢球识别 | 蓝=`±10mm` 内，黄=偏差较大，灭=无钢球 |
| 5 | 摆杆平衡 | 蓝呼吸=调节中，绿=稳定，灭=未启用 |
| 6 | 赛题动作 | 紫呼吸=执行中，绿/红按动作编号闪烁=完成/失败 |
| 7 | 安全总览 | 绿=控制允许，黄=仅观测，红=急停、失联或故障 |
