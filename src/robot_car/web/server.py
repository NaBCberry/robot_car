...
</style></head><body><header><strong>管槽钢球标定</strong><a href="/">监控</a><a class="active" href="/calibration">手工标定</a><span id="fps">帧率读取中</span></header><main class="layout"><section class="feed"><div class="stage"><img id="frame" src="/video_feed" alt="实时相机画面"><canvas id="canvas"></canvas></div></section><aside>
...
const frame=document.getElementById('frame'),canvas=document.getElementById('canvas'),ctx=canvas.getContext('2d');
const setupText=['点击：管槽 ROI 左上角。','点击：管槽 ROI 右下角。','点击：管槽中心 O 点。'];const setupKeys=['左上','右下','中心 O'];const scaleMarks=Array.from({length:25},(_,index)=>index*10-120);let setupPoints=[],scalePoints=[],sourceSize={width:0,height:0};
...
function draw(){ctx.clearRect(0,0,canvas.width,canvas.height);if(!frame.naturalWidth)return;const sx=canvas.width/frame.naturalWidth,sy=canvas.height/frame.naturalHeight;ctx.lineWidth=2;ctx.font='12px Arial';setupPoints.forEach((point,index)=>drawPoint(point.x*sx,point.y*sy,'#ffd24a',setupKeys[index]));if(setupPoints.length>=2){const [a,b]=setupPoints;ctx.strokeStyle='#55d5a2';ctx.strokeRect(a.x*sx,a.y*sy,(b.x-a.x)*sx,(b.y-a.y)*sy)}if(setupPoints.length===3){const point=setupPoints[2];ctx.strokeStyle='#ff9f43';ctx.beginPath();ctx.moveTo(point.x*sx,0);ctx.lineTo(point.x*sx,canvas.height);ctx.stroke()}scalePoints.forEach((point,index)=>drawPoint(point.x*sx,point.y*sy,'#5ec9ef',labelFor(point.position_mm)));if(scalePoints.length&&scalePoints.length<scaleMarks.length){const point=scalePoints[scalePoints.length-1];ctx.fillStyle='#fff';ctx.fillText('下一个：'+labelFor(scaleMarks[scalePoints.length]),point.x*sx+8,point.y*sy+16)}}
canvas.addEventListener('click',event=>{if(!frame.naturalWidth||scalePoints.length===scaleMarks.length)return;const rect=canvas.getBoundingClientRect(),point={x:(event.clientX-rect.left)*frame.naturalWidth/rect.width,y:(event.clientY-rect.top)*frame.naturalHeight/rect.height};if(setupPoints.length<3)setupPoints.push(point);else scalePoints.push({...point,position_mm:scaleMarks[scalePoints.length]});showStatus('');update()});document.getElementById('reset').addEventListener('click',()=>{setupPoints=[];scalePoints=[];showStatus('');update()});frame.addEventListener('load',sync);window.addEventListener('resize',sync);
document.getElementById('save').addEventListener('click',async()=>{const length=Number(document.getElementById('length').value);if(!Number.isFinite(length)){showStatus('请输入有效的管槽长度。','error');return}if(!frame.naturalWidth||!frame.naturalHeight||!sourceSize.width||!sourceSize.height){showStatus('尚未得到相机原始图像尺寸，请等待视频和帧率显示正常。','error');return}const sx=sourceSize.width/frame.naturalWidth,sy=sourceSize.height/frame.naturalHeight,[a,b,center]=setupPoints;const payload={roi_xyxy:[a.x*sx,a.y*sy,b.x*sx,b.y*sy],center_x_px:center.x*sx,center_y_px:center.y*sy,tube_length_mm:length,axis_direction:Number(document.querySelector('input[name="direction"]:checked').value),axis_points:scalePoints.map(point=>({position_mm:point.position_mm,pixel_x:point.x*sx}))};try{const response=await fetch('/api/roller_balance/calibration',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const result=await response.json();if(!response.ok)throw new Error(result.error||'保存失败');showStatus('已保存 25 个刻度。请重启 visiond 后再启用平衡插件。','ok')}catch(error){showStatus(error.message,'error')}});
