...
<div class="label">状态</div><div class="value" id="target">未检测到钢球</div><div class="label">相机帧率</div><div class="value" id="camera-fps">-</div><div class="label">网页预览帧率</div><div class="value" id="preview-fps">-</div><div class="label">位置</div><div class="value" id="position">-</div><div class="label">目标位置</div><div class="value" id="setpoint">-</div><div class="label">偏差</div><div class="value" id="error">-</div><div class="label">速度</div><div class="value" id="velocity">-</div><div class="label">加速度</div><div class="value" id="acceleration">-</div><div class="label">置信度</div><div class="value" id="confidence">-</div><div class="label">帧号</div><div class="value" id="frame-id">-</div></aside></main><script>
...
const frame=document.getElementById('frame'),canvas=document.getElementById('canvas'),ctx=canvas.getContext('2d');const stepText=['点击：管槽 ROI 左上角。','点击：管槽 ROI 右下角。','点击：管槽中心 O 点。','确认选点和长度后保存。'];const keys=['左上','右下','中心 O'];let points=[],sourceSize={width:0,height:0};
...
document.getElementById('save').addEventListener('click',async()=>{const length=Number(document.getElementById('length').value);if(!Number.isFinite(length)){showStatus('请输入有效的管槽长度。','error');return}if(!frame.naturalWidth||!frame.naturalHeight||!sourceSize.width||!sourceSize.height){showStatus('尚未得到相机原始图像尺寸，请等待视频和帧率显示正常。','error');return}const sx=sourceSize.width/frame.naturalWidth,sy=sourceSize.height/frame.naturalHeight,[a,b,center]=points;const payload={roi_xyxy:[a.x*sx,a.y*sy,b.x*sx,b.y*sy],center_x_px:center.x*sx,center_y_px:center.y*sy,tube_length_mm:length,axis_direction:Number(document.querySelector('input[name="direction"]:checked').value)};try{const response=await fetch('/api/roller_balance/calibration',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const result=await response.json();if(!response.ok)throw new Error(result.error||'保存失败');showStatus('已保存：'+JSON.stringify(result.roller_balance)+'。请重启 visiond 后再启用平衡插件。','ok')}catch(error){showStatus(error.message,'error')}});async function refreshFps(){try{const status=await fetch('/api/status',{cache:'no-store'}).then(r=>r.json());sourceSize={width:Number(status.frame_width||0),height:Number(status.frame_height||0)};document.getElementById('fps').textContent='相机 '+Number(status.camera_fps||0).toFixed(1)+' fps / 网页 '+Number(status.preview_fps||0).toFixed(1)+' fps'}catch(_){document.getElementById('fps').textContent='帧率不可用'}}refreshFps();setInterval(refreshFps,500);sync();update();
</script></body></html>"""
