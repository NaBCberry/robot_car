...
from robot_car.roller_control.icm42688 import (ACCEL_CONFIG0, GYRO_CONFIG0, PWR_MGMT0,
                                                Icm42688, WHO_AM_I)
...
class Icm42688Tests(unittest.TestCase):
    def test_open_verifies_identity_and_reads_sample(self):
        fake = FakeSpi(sample=[0, 1, 0, 2, 0, 3, 255, 252, 0, 5, 255, 250, 0, 7])
        sensor = Icm42688(1, 0, spi_factory=lambda: fake, monotonic=lambda: 12.5)
        sensor.open()
