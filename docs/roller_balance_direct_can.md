...
当前配置使用 `i2c-0`、地址 `0x69`。ICM42688 的 `CS` 必须保持高电平以选择 I2C，
`SCL/SDA` 接 RDK X5 的 I2C0 引脚，传感器必须装在会随摆杆转动的部件上，否则只能测到车架姿态，不能闭合摆杆角度环。
若模块的 AD0 地址脚为低电平，则把 `i2c_address` 改为 `0x68`。

正常探测时，读取寄存器 `0x75` 的结果是 `0x47`。运行 `probe_icm42688.sh` 前应确认
`/dev/i2c-0` 存在；不要在 `WHO_AM_I` 仍为 `0x00` 时启用本链路。
