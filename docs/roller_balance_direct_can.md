...
- HXY ICM-42688P 身份寄存器必须读取为 `0x6A`；识别失败时进程在使能电机前退出。
...
若模块接在 SPI1 的 `CSN1`，当前目标就是 `spi1.1`；HXY 手册规定 SPI 模式为 3。传感器必须装在会随摆杆转动的部件上，
否则只能测到车架姿态，不能闭合摆杆角度环。

正常探测时，读取寄存器 `0x01` 的结果是 `0x6A`，首个 MOSI 字节应为 `0x03`。当前现场探测的 `spi1.0` 和 `spi1.1`
