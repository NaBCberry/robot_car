...
`CAPTURE` 后读取 Y42 多圈实时位置，扣除 `2°` 安全余量，并在明确传入 `--apply` 时写入软限位。
Y42 的位置命令和实时位置均可超过 `360°`，因此软限位必须保持该多圈坐标，不能对读数取模。
脚本同时显示单圈编码器角；若电机带减速箱，可配置 `position_deg_per_output_deg` 仅用于显示
输出轴换算值，不会改变 CAN 控制坐标。

采集后，脚本会询问曲轴是否已锁紧；确认 `LOCKED` 后可再输入 `HOME`，脚本倒计时 2 秒、以
`5 RPM` 回到 Y42 坐标零点，并轮询位置确认到位。
